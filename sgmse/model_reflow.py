...
class ScoreModel_reflow(pl.LightningModule):
    @staticmethod
    def add_argparse_args(parser):
        ...
        parser.add_argument("--loss_type", type=str, default="rectified_flow", help="The type of loss function to use.")
    ...
    def _loss(self, forward_out, x_t, z, t, mean, x):
        ...
        if self.loss_type == "score_matching":
            ...
            # #if torch.allclose(forward_out, torch.complex(forward_out.real, forward_out.imag)):
            # #    print(True)
            loss = real_mse + imag_mse

            # loss = torch.square(torch.nn.functional.mse_loss(torch.abs(forward_out), torch.abs(x_t)))
    ...
    def _step(self, batch, batch_idx):
        ...
        t = torch.sigmoid(nt)
        # print(t)
        # mean, _ = self.sde.marginal_prob(x, t)
        # sigma = self.sde.sigma(t)
        #x_T = torch.randn_like(x)
        texp = t.view([b, *([1] * len(x.shape[1:]))])
        x_t = (1 - texp) * x + texp * z
        target_drift = z - x
        forward_out = self(x_t, y, t)  # 模型前向传播

        cosine_similarity = torch.nn.functional.cosine_similarity(forward_out.real.flatten(1), target_drift.real.flatten(1), dim=1) 
        avg_cosine_similarity = cosine_similarity.mean().item()
        print("余弦相似度:", avg_cosine_similarity)
        relative_error = torch.abs(torch.norm(target_drift) - torch.norm(forward_out)) / torch.norm(forward_out)
        print("数值误差:", relative_error.item())

        loss = self._loss(forward_out = forward_out, x_t = target_drift, z = None, t = t, mean = None, x = None)  # 计算损失
        return loss

    def training_step(self, batch, batch_idx):
        loss = self._step(batch, batch_idx)
        current_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        with open("/home/icdm/yg/sgmse+flow2/train_loss_value.txt", "a") as f:
            f.write(f"{current_time} - Loss: {loss}\n")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # b = cl.size(0)
        # nt = torch.randn((b,)).to(cl.device)
        # t = torch.sigmoid(nt)
        # # print(t)
        # # mean, _ = self.sde.marginal_prob(x, t)
        # # sigma = self.sde.sigma(t)
        # texp = t.view([b, *([1] * len(cl.shape[1:]))])
        # cl_T = torch.randn_like(a)
        # cl_t = (1 - texp) * cl + texp * cl_T
        # target_drift = cl_T - cl
        # forward_out = self(cl_t, no, t)  # 模型前向传播
        ...
        if self.num_eval_files != 0:
            ...
            for (clean_file, noisy_file) in zip(clean_files, noisy_files):
                ...
                if sr_x != 16000:
                    ...
                    x_16k = x
                # Enhance the noisy speech,用于测试，打印采样过程中每一步的与干净语音x的pesq值
                ...
                # print("x_hat_16k的形状：", x_hat_16k.shape)
                # print("x_16k的形状", x_16k.shape) 
                # x_hat_16k_tensor = torch.tensor(x_hat_16k)
                # x_16k_tensor = torch.tensor(x_16k)

                # cosine_similarity = torch.nn.functional.cosine_similarity(x_hat_16k_tensor.flatten(), x_16k_tensor.flatten(), dim=0)
                # print("余弦相似性:", cosine_similarity.item())
            ...
            with open("/home/icdm/yg/sgmse+flow2/result.txt", "a") as f:
                f.write(f"{current_time} - PESQ Avg: {pesq_avg}, SI-SDR: {si_sdr_avg}, ESTOI: {estoi_avg}\n")
    ...
    def sample(self, clean_16k, cond, sr_clean, score_model, sample_steps=6, cfg=2.0, pesq_func=pesq):
        #对条件y数据预处理部分
        # target_len = 32640
        # current_len = cond.size(-1)
        # T_orig = cond.size(1)
        ...
        # else:
        #     cond = F.pad(cond, (pad//2, pad//2+(pad%2)), mode='constant')
        # cond = torch.unsqueeze(self._forward_transform(self._stft(cond.cuda())), 0)
        # cond = cond.abs()**0.5 * torch.exp(1j * cond.angle())
        ...
        with torch.no_grad():
            ...
            #y_re = self.to_audio(y.squeeze(), T_orig)
            #y_re = y_re * norm_factor
            #y_re = y_re.squeeze().cpu().numpy()
            #cond = cond.squeeze().numpy()
            #y_re = resample(y_re, orig_sr=sr_clean, target_sr=16000).squeeze()  
                #enhance时注释下面五行
            x_hat_16k = resample(x_hat, orig_sr=sr_clean, target_sr=16000).squeeze()   

            pesq_value = pesq(16000, clean_16k, x_hat_16k, 'wb') 
            if i == 1:
                with open("/home/icdm/yg/sgmse+flow2/pesq_values.txt", "a") as f:
                    f.write(f"Step {i}: PESQ Value: {pesq_value:.2f}, Type: {type(pesq_value)}\n")
        return x_hat
    
    def samplefortest(self, clean_16k, cond, sr_clean, score_model, sample_steps=50, cfg=2.0, pesq_func=pesq):
        T_orig = cond.size(1) 
        norm_factor = cond.abs().max().item()
        cond = cond / norm_factor
        cond = torch.unsqueeze(self._forward_transform(self._stft(cond.cuda())), 0)
    ...
    def sampleforreflow(self, clean_16k, cond, sr_clean, score_model, sample_steps=50, cfg=2.0, pesq_func=pesq):
        T_orig = cond.size(1) 
        norm_factor = cond.abs().max().item()
        cond = cond / norm_factor
        cond = torch.unsqueeze(self._forward_transform(self._stft(cond.cuda())), 0)
        ...
        z = torch.randn_like(cond).to(cond.device)
        noise = z
        noise = self.to_audio(noise.squeeze(), T_orig)
    ...
    def enhance(self, y, sampler_type="pc", predictor="reverse_diffusion",
        ...
        **kwargs
        ...
        elif self.sde.__class__.__name__ == 'RectifiedFlowSDE':
            # 添加 RectifiedFlowSDE 的采样逻辑
            sampler = self.get_ode_sampler(Y.cuda(), N=N, **kwargs)
